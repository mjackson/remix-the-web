...
const WASM_OUT = resolve(__dirname, '..', 'src', 'llhttp');

// These are defined by build environment
const WASM_CC = process.env.WASM_CC || 'clang';
let WASM_CFLAGS =
  process.env.WASM_CFLAGS || '--sysroot=/usr/share/wasi-sysroot -target wasm32-unknown-wasi';
...
// These are relevant for llhttp and should not be overridden
...
if (process.argv[2] === '--docker') {
  ...
  cmd += ` --mount type=bind,source=${ROOT}/src/llhttp,target=/home/node/build/src/llhttp \
           --mount type=bind,source=${ROOT}/scripts,target=/home/node/build/scripts \
           --mount type=bind,source=${ROOT}/deps,target=/home/node/build/deps \
           -t ${WASM_BUILDER_CONTAINER} node scripts/build-wasm.js`;
  console.log(`> ${cmd}\n\n`);
...
if (hasOptimizer) {
  execSync(
    `${WASM_OPT} ${WASM_OPT_FLAGS} -o ${join(WASM_OUT, 'llhttp.wasm')} ${join(
      WASM_OUT,
      'llhttp.wasm',
    )}`,
    { stdio: 'inherit' },
...
execSync(
  `${WASM_CC} ${WASM_CFLAGS} -msimd128 ${WASM_LDFLAGS} \
${join(WASM_SRC, 'src')}/*.c \
-I${join(WASM_SRC, 'include')} \
...
if (hasOptimizer) {
  execSync(
    `${WASM_OPT} ${WASM_OPT_FLAGS} --enable-simd -o ${join(
      WASM_OUT,
      'llhttp_simd.wasm',
