...
Patterns may specify any combination of URL properties: protocol, hostname, pathname, and search.
...
A wildcard matches dynamic values within a segment and is written as `*`:
...
'/products/*/edit';
// /products/laptop/edit → {}
// /products/phone/edit → {}

'https://*.remix.run/store';
// https://shop.remix.run/store → {}
// https://api.remix.run/store → {}
```

A pattern can contain multiple wildcards even within the same segment:
...
Wildcards in hostnames match `/[^.]*/` — "anything except `.`":

```ts
'://*.example.com';
// https://api.example.com → {}
// https://shop.example.com → {}
// ❌ Does NOT match: https://sub.api.example.com

'://api-*.example.com';
// https://api-v1.example.com → {}
// https://api-staging.example.com → {}
// ❌ Does NOT match: https://api.v1.example.com
```

Wildcards in pathnames match `/[^/]*/` — "anything except `/`":

```ts
'/files/*';
// /files/document → {}
// /files/image.jpg → {}
// ❌ Does NOT match: /files/folder/document

'/api/*/users';
// /api/v1/users → {}
// /api/v2/users → {}
// ❌ Does NOT match: /api/v1/beta/users
```

### Params

A param is like a wildcard, but it also binds the matching value to a name.
It is written as `:` followed by a [JavaScript identifier](#javascript-identifier) that acts as its name:
...
// /products/sku@123 → { id: 'sku@123' }
// /products/item_with_underscores → { id: 'item_with_underscores' }
